...
- **An `npm unpublish --force`-ed version number is PERMANENTLY consumed on npmjs** — republishing it fails with `E400 Cannot publish over previously published version`. Never reuse an unpublished version: 0.1.0 was consumed this way (accidental publish during rc1 cleanup); the first stable release is **0.1.1**. Also: the first-ever publish of an empty package record gets `latest` even with `--tag next` (transient, until the real release re-tags it).
